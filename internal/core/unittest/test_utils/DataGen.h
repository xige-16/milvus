...
GenVecIndexing(int64_t N, int64_t dim, const float* vec) {
    ...
    auto indexing = std::make_shared<Index::VectorMemIndex>(knowhere::IndexEnum::INDEX_FAISS_IVFFLAT, knowhere::metric::L2, IndexMode::MODE_CPU);
...
GenScalarIndexing(int64_t N, const T* data) {
    auto dataset = knowhere::GenDataset(N, 1, data);
    if constexpr (std::is_same_v<T, std::string>) {
        auto indexing = Index::CreateStringIndexSort();
        indexing->BuildWithDataset(dataset);
        return indexing;
    } else {
