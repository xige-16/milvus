...
DeserializeRemoteFileData(PayloadInputStream* input_stream) {
    ...
    switch (header.event_type_) {
        ...
        case EventType::IndexFileEvent: {
            ...
            index_meta.build_id = std::stoi(extras[INDEX_BUILD_ID_KEY]);
            AssertInfo(extras.find(BINLOG_FILE_KEY) != extras.end(), "index name not exist");
            index_meta.key = extras[BINLOG_FILE_KEY];
