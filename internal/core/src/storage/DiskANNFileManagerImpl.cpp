...
DiskANNFileManagerImpl::AddFile(const std::string& file) noexcept {
    ...
    auto ret_size = local_chunk_manager.Read(file, buf.get(), fileSize);
    ...
    for (int offset = 0; offset < localPayloadSize; slice_num++) {
        auto batch_size = std::min(milvus::config::KnowhereGetIndexSliceSize(), localPayloadSize - offset);
        ...
        remote_chunk_manager.Write(objectKey, subFile.data(), subFile.size());
        offset += batch_size;

        // record remote file to save etcd
        remote_paths_.emplace_back(objectKey);
    }
    FILEMANAGER_CATCH
    FILEMANAGER_END
...
DiskANNFileManagerImpl::GetRemoteObjectPrefix() {
    return std::string(INDEX_ROOT_PATH) + "/" + std::to_string(index_meta_.build_id) + "/" +
...
DiskANNFileManagerImpl::GetLocalObjectPrefix() {
    return milvus::GenLocalIndexPathPrefix(index_meta_.build_id, index_meta_.index_version);
}
