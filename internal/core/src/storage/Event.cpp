...
BaseEventData::Serialize() {
    auto data_type = field_data->GetDataType();
    std::shared_ptr<PayloadWriter> payload_writer;
    if (milvus::datatype_is_vector(data_type)) {
        payload_writer = std::make_unique<PayloadWriter>(data_type, field_data->GetDim());
    } else {
        payload_writer = std::make_unique<PayloadWriter>(data_type);
    }
    if (datatype_is_string(data_type)) {
        auto str_datas = static_cast<FieldDataImpl<std::string>*>(field_data.get());
        for (size_t index = 0; index < str_datas->length(); ++index) {
            auto str = str_datas->get_value(index);
            payload_writer->add_one_string_payload(str.c_str(), str.size());
        }
    } else {
        auto payload = Payload{data_type, static_cast<const uint8_t*>(field_data->Data()), field_data->GetNumRows(),
                               field_data->GetDim()};
...
LocalInsertEvent::Serialize() {
    int row_num = field_data->GetNumRows();
    int dimension = field_data->GetDim();
    int payload_size = field_data->GetDataSize();
...
LocalIndexEvent::Serialize() {
    index_size = field_data->GetDataSize();
