...
SegmentGrowingImpl::LoadFieldData(const LoadFieldDataInfo& infos) {
    ...
    for (auto& field_info : infos.field_infos) {
        ...
        if (info.field_id == TimestampFieldID.get()) {
            // step 2: sort timestamp
            // query node already guarantees that the timestamp is ordered, avoid field data copy in c++
        ...
        if (info.field_id == RowFieldID.get()) {
            insert_record_.row_ids_.set_data_raw(reserved_offset, field_datas);
            continue;
        }

        insert_record_.get_field_data_base(FieldId(info.field_id))->set_data_raw(reserved_offset, field_datas);
        if (info.field_id == primary_field_id.get()) {
