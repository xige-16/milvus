...
int64_t
VecIndexCreator::dim() {
    auto dimension = get_config_by_name<std::string>(knowhere::meta::DIM);
...
VecIndexCreator::get_build_index_info() const {
    ...
    if (config_.contains(Index::COLLECTION_ID)) {
        index_info.collection_id = std::stol(get_config_by_name<std::string>(Index::COLLECTION_ID).value());
    }

    if (config_.contains(Index::PARTITION_ID)) {
        index_info.partition_id = std::stol(get_config_by_name<std::string>(Index::PARTITION_ID).value());
    }

    if (config_.contains(Index::SEGMENT_ID)) {
        index_info.segment_id = std::stol(get_config_by_name<std::string>(Index::SEGMENT_ID).value());
    }

    if (config_.contains(Index::FIELD_ID)) {
        index_info.field_id = std::stol(get_config_by_name<std::string>(Index::FIELD_ID).value());
    }

    if (config_.contains(Index::INDEX_BUILD_ID)) {
        index_info.index_build_id = std::stol(get_config_by_name<std::string>(Index::INDEX_BUILD_ID).value());
    }

    if (config_.contains(Index::INDEX_ID)) {
        index_info.index_id = std::stol(get_config_by_name<std::string>(Index::INDEX_ID).value());
    }

    if (config_.contains(Index::INDEX_VERSION)) {
        index_info.index_version = std::stol(get_config_by_name<std::string>(Index::INDEX_VERSION).value());
    }
    return index_info;
}
