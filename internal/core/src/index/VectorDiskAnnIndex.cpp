...
VectorDiskAnnIndex<T>::Load(const BinarySet& binary_set /* not used */, const Config& config) {
    // TODO :: load raw data from remote
    //    auto& remote_chunk_manager = storage::MinioChunkManager::GetInstance();
    //    auto remote_path_prefix = file_manager_->GetRemoteObjectPrefix();
    //    auto paths = remote_chunk_manager.ListWithPrefix(remote_path_prefix);
...
VectorDiskAnnIndex<T>::BuildWithDataset(const DatasetPtr& dataset, const Config& config) {
    ...
    auto local_data_path = milvus::GenIndexRawDataPathPrefix(std::stol(segment_id.value()), std::stol(field_id.value())) + "raw_data";
    build_config.data_path = local_data_path;

    auto& local_chunk_manager = storage::LocalChunkManager::GetInstance();
    auto num = knowhere::GetDatasetRows(dataset);
    auto dim = knowhere::GetDatasetDim(dataset);
    std::cout << " build disk ann, num = " << num << ", dim = " << dim << std::endl;
//    auto data_size = num * dim * sizeof(float);
    auto raw_data = const_cast<void*>(knowhere::GetDatasetTensor(dataset));
//    //TODO:: ugly
//    storage::LocalInsertEvent insert_event;
//    auto total_size = sizeof(insert_event.row_num) + sizeof(insert_event.dimension) + data_size;
//    std::vector<uint8_t> disk_raw_data(total_size);
//    int offset = 0;
//    memcpy(disk_raw_data.data() + offset, &num, sizeof(insert_event.row_num));
//    offset += sizeof(insert_event.row_num);
//    memcpy(disk_raw_data.data() + offset, &dim, sizeof(insert_event.dimension));
//    offset += sizeof(insert_event.dimension);
//    memcpy(disk_raw_data.data() + offset, raw_data, data_size);

//    local_chunk_manager.Write(local_data_path, (char*)&num, sizeof(uint32_t));
//    local_chunk_manager.Write(local_data_path, (char*)&dim, sizeof(uint32_t));
//    local_chunk_manager.Write(local_data_path, (char*)raw_data, sizeof(float) * num * dim);

    storage::WriteRawDataToDisk(local_data_path,reinterpret_cast<float*>(raw_data), num, dim);
...
VectorDiskAnnIndex<T>::parse_query_config(Config& config) {
    ...
    AssertInfo(topK.has_value(), "param " + std::string(knowhere::meta::TOPK) + "is empty");
...
VectorDiskAnnIndex<T>::parse_config(Config& config) {
    auto stoi_closure = [](const std::string_view& s) -> uint32_t { return std::stoi(std::string(s)); };
    ...
    CheckParameter<int>(config, DISK_ANN_MAX_DEGREE, stoi_closure, std::optional{24});
    CheckParameter<int>(config, DISK_ANN_BUILD_LIST, stoi_closure, std::optional{64});
