...
class VectorDiskAnnIndex : public VectorIndex {
 public:
    ...
    Serialize(const Config& config) override {
        auto remote_paths = file_manager_->GetRemotePaths();
        BinarySet binary_set;
        for (auto& file : remote_paths) {
            // TODO:: test
//            std::shared_ptr<uint8_t[]> data(new uint8_t [20]());
            binary_set.Append(file, nullptr, 0);
        }

        std::shared_ptr<uint8_t[]> data(new uint8_t [20]());
        binary_set.Append("index", data, 20);
