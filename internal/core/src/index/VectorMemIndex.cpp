...
VectorMemIndex::BuildWithDataset(const DatasetPtr& dataset, const Config& config) {
    ...
    // std::cout << "Konwhere BuildWithoutIds index_config_ is " << index_config_ << std::endl;
...
VectorMemIndex::parse_config(Config& config) {
    ...
    /***************************** meta *******************************/
    CheckParameter<int>(config, knowhere::meta::SLICE_SIZE, stoi_closure, std::optional{config::KnowhereGetIndexSliceSize()});
