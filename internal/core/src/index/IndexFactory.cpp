...
IndexFactory::CreateVectorIndex(const BuildIndexInfo& build_index_info) {
    ...
    storage::FieldDataMeta field_meta{build_index_info.collection_id, build_index_info.partition_id,
    ...
    if (is_in_disk_list(index_type)) {
        switch (data_type) {
            case DataType::VECTOR_FLOAT: {
                auto file_manager = std::make_shared<knowhere::DiskANNFileManagerImpl>(field_meta, index_meta);
